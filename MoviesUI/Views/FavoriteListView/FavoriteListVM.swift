...
class FavoriteListVM: ObservableObject {
    ...
    func getMovieFavoritelist() {
        ...
        self.repo.getMovieFavoritelist(page: moviesPage, sessionId: sessionId, accountId: accountId)
            .handleEvents(receiveSubscription: { (sub) in
                print("sub", sub)
            }, receiveOutput: { (output) in
                print("output getMovieFavoritelist", output)
